...
if st.session_state.logged_in:
    ...
    st.sidebar.info("Please log in or sign up on the main page.")


# Main Page
st.title("👨‍⚕️ GenAI-Powered Medical Diagnosis")
st.markdown(
    "This application uses a FastAPI backend with a LangChain/GenAI stack "
    "to help patients get a diagnosis from their reports and allow doctors to access their patients' diagnosis history."
)


def show_dashboard():
    """Render dashboard for logged-in users based on role."""
...
if not st.session_state.logged_in:
    ...
    if st.button("Login"):
        if login_username and login_password:
            ...
            if status_code == 200:
                ...
                st.session_state.login_complete = False  # Wait for user to confirm continue
                st.success("Logged in successfully! Please click Continue to proceed.")
            else:
                st.error(f"Login failed: {data.get('detail', 'Unknown error')}")
        else:
            st.warning("Please enter a username and password.")

    # Show Continue button after login success
    if st.session_state.get("logged_in") and not st.session_state.get("login_complete", False):
        if st.button("Continue"):
            st.session_state.login_complete = True

    # Only show dashboard after Continue clicked
    if st.session_state.get("logged_in") and st.session_state.get("login_complete", False):
        show_dashboard()
    ...
    # User is already logged in and login_complete True, show dashboard
    if st.session_state.login_complete:
        show_dashboard()
    else:
        # Show Continue button to confirm login
        if st.button("Continue"):
            st.session_state.login_complete = True
            show_dashboard()
        else:
            st.info("Please click Continue to access your dashboard.")
